...
class CircleAngerBar( Window, Singleton ):

	def __init__( self ):
		gui = GUI.load("guis/otheruis/circlehpbar/circleangerbar.gui")
	...
	def __initialize( self, gui ):
		...
		self.__pyHPBar = CircleShader(gui.anger_ring)

	def __onAngerChanged( self, hpPercent ):
	...
	def cls_update( CLS, hpPercent ):
		if CLS.insted:
			CLS.inst.__onAngerChanged( hpPercent )
