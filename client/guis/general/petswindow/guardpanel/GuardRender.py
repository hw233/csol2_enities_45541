...
class GuardRender( AdjModelRender ) :
	__cc_config = "config/client/uimodel_configs/guard.py"
	...
	def resetModel( self, className, bgCreate = True ) :
		...
		if className is None :
			...
			skillId = self.__getSkillID( className )
			skill = skills.getSkill( skillId )
			level = skill.getLevel()
			guardInfo = guardInfosLoader.getGuardInfo( className, level )
	...
	def getViewInfos( self ) :
		...
		获取所有模型信息
		"""
		skillId = self.__getSkillID( self.cfgKey_ )
		skill = skills.getSkill( skillId )
		level = skill.getLevel()
		guardInfo = guardInfosLoader.getGuardInfo( self.cfgKey_, level )
		name = guardInfo.name
		pos = self.modelPos
		pitch = self.pitch
		yaw = self.yaw
