...
class NPCCityMaster( GameObject ):
	def __init__( self ):
		...
		self.modelScale = 1.0
		self.equipEffects = []
		self.visibleRules = [csdefine.VISIBLE_RULE_BY_PLANEID,csdefine.VISIBLE_RULE_BY_FLASH]
	...
	def setName( self, uname ):
		...
		"""
		self.uname = uname
		ECenter.fireEvent( "EVT_ON_CITYMASTER_NAME_CHANGED", self.id, uname )
	...
	def setTitle( self, title ):
		# define method
		self.title = title
		ECenter.fireEvent( "EVT_ON_CITYMASTER_TITLE_CHANGED", self.id, title )
	...
	def setTongName( self, tongName ):
		...
		ECenter.fireEvent( "EVT_ON_CITYMASTER_TONGNAME_CHANGED", self.id, tongName )
