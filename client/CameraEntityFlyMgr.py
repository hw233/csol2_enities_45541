...
#客户端镜头实体空中飞行管理 按照配置路径进行曲线运动 edit by wuxo 2013-12-25
...
class CameraEntityFlyMgr:
	__instance = None

	def __init__( self ):
		assert CameraEntityFlyMgr.__instance is None
	...
	def instance( self ):
		if self.__instance is None:
			self.__instance = CameraEntityFlyMgr()
	...
	def startRoleFly( self, ce, path, posCount = 0, isLoop = 1,callback = None ):
		self.cameraEntity = ce
		self.data  = None
		self.paths     = None
		self.cb        = None
		...
		#self.cameraEntity.turnaround( matrix)
		#首先由当前位置移动到飞行路径的第一个点
		self.gotoFisrtPos( dPos )
		
	def gotoFisrtPos( self, dPos ):
		self.cameraEntity.moveTo( dPos, callback = self.__onMoveSuccess )	
		
		
	def __onMoveSuccess( self ):
		"""
		移动回调
		"""
		self.roleFly()
	...
	def roleFly(self):
			...
			if self.isLoop and data == self.posDatas[-1]:	#如果循环
				...
				def moveOver():
					...
					if callable(self.callback):
						self.callback()	
				pos = self.posDatas[-1][3]
				self.cameraEntity.moveTo( pos, callback = moveOver )
			return
		pos0 = data[0] 
		pos1 = data[1] 
		pos2 = data[2]
		...
		self.cameraEntity.moveTo( pos, move_speed )
...
ceFlyMgr = CameraEntityFlyMgr.instance()