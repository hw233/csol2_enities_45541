...
class MonsterCampFHLTAltar( Monster ):
	...
	def __init__( self ):
		...
		self.setEntityType( csdefine.ENTITY_TYPE_CAMP_FENG_HUO_ALTAR )
		if self.getCurrentSpaceBase():
			self.getCurrentSpaceBase().cell.addAltarNum( self.ownCamp )
	...
	def onDie( self, killerID ):
		...
		if killer.isEntityType( csdefine.ENTITY_TYPE_CAMP_XIAN_FENG ):
			self.getCurrentSpaceBase().cell.decAltarNum( self.ownCamp )
