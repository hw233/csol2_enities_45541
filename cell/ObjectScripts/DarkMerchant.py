...
class DarkMerchant( Chapman.Chapman ):
	"""
	"""
	...
	def onSellItems( self, selfEntity, playerEntity, items, argIndices, argAmountList ):
		...
		销售某类物品事件
		"""
		playerEntity.buyItemFromDarkMerchant( selfEntity, items, argIndices, argAmountList )
	...
	def buyFrom( self, selfEntity, playerEntity, argUid, argAmount ):
		...
		"""
		#黑市商人改成不收购物品
		playerEntity.statusMessage( csstatus.MERCHANT_NO_BUY_THIS_ITEM )
		# 卖大量跟卖一（个）组不应该用一样接口，但目前不想大改，先这样做 -pj
		#playerEntity.sellItemToDarkMerchant( selfEntity, [argUid], [argAmount] )
	...
	def buyArrayFrom( self, selfEntity, playerEntity, argUidList, argAmountList ):
		...
		#if playerEntity.actionSign( csdefine.ACTION_FORBID_TALK ):				#判断是否能跟Merchant对话 完成操作
		#	return
		#playerEntity.sellItemToDarkMerchant( selfEntity, argUidList, argAmountList )
