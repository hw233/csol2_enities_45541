...
FENG_HUO_LIAN_TIAN_BATTLE_FLAG		= "30112306"
ZHAO_HUAN_WU_ZI						= "30112304"
JI_FEN_WU_ZI						= "30112305"
LIGHT_WALL_CLASSNAME				= "20254054"
XIAN_FENG_GUAI_DICT					= { 0:{"left":"20129470", "mid":"20129471", "right":"20129472"}, 1:{"left":"20129475", "mid":"20129474", "right":"20129473"}}
...
ARG_CAMP_NOTICE_LIST				= [ 102, 103, 104 ]
ARG_DELAY_CLOSE						= 105
...
class CCProtectProcess( CopyContent ):
	...
	def onContent( self, spaceEntity ):
		...
		for e in spaceEntity._players:
			...
			e.client.campFHLTProtectTime( PROTECT_TIME )
	...
	def onEnter( self, spaceEntity, baseMailbox, params ):
		...
		if protectTime:
			baseMailbox.client.campFHLTProtectTime( int( protectTime - time.time() ) )
...
class CCCombatProcess( CopyContent ):
	...
	def onContent( self, spaceEntity ):
		...
		"""
		for e in spaceEntity._players:
			e.cell.setSysPKMode( csdefine.PK_CONTROL_PROTECT_CAMP )
		...
		if BigWorld.globalData.has_key( "campFengHuoOverTime" ):
			spaceEntity.addTimer( int( BigWorld.globalData[ "campFengHuoOverTime" ] - time.time() ) - NOTICE_TIME_LIST[0], 0, ARG_CAMP_NOTICE_LIST[0] )
			spaceEntity.addTimer( int( BigWorld.globalData[ "campFengHuoOverTime" ] - time.time() ) - NOTICE_TIME_LIST[1], 0, ARG_CAMP_NOTICE_LIST[1] )
			spaceEntity.addTimer( int( BigWorld.globalData[ "campFengHuoOverTime" ] - time.time() ) - NOTICE_TIME_LIST[2], 0, ARG_CAMP_NOTICE_LIST[2] )
	...
	def onEnter( self, spaceEntity, baseMailbox, params ):
		...
		内容期间，角色进入
		"""
		baseMailbox.cell.setSysPKMode( csdefine.PK_CONTROL_PROTECT_CAMP )
	...
	def onTimer( self, spaceEntity, id, userArg ):
		...
		elif userArg in ARG_CAMP_NOTICE_LIST:
			i = ARG_CAMP_NOTICE_LIST.index( userArg )
			spaceEntity.noticeCampSituation( NOTICE_TIME_LIST[ i ] )
	...
	def spawnTowerAndAltar( self, spaceEntity ):
		...
		for i in tower_and_altar:
			if i == "left_tower_and_altar":
				camp = spaceEntity.params[ "left" ]
			elif i == "right_tower_and_altar":
				camp = spaceEntity.params[ "right" ]
	...
	def spawnMonster( self, spaceEntity ):
		...
		spaceEntity.noticePlayers( cschannel_msgs.CAMP_FENG_HUO_JI_FEN_WU_ZI_IS_APPEARING, [] )
		spaceEntity.noticePlayers( cschannel_msgs.CAMP_FENG_HUO_ZHAO_HUAN_WU_ZI_IS_APPEARING, [] )
		spaceEntity.addTimer( WU_ZI_JIAN_GE_TIME, 0, ARG_WU_ZI)
...
class SpaceCopyCampFengHuoLianTian( SpaceCopyTemplate ):
	"""
	阵营烽火连天
	"""
	def __init__( self ):
	...
	def packedDomainData( self, entity ):
		...
		return { 'camp' : entity.getCamp(), "ename" : entity.getName(), "dbid" : entity.databaseID }
	...
	def packedSpaceDataOnEnter( self, entity ):
		...
		packDict[ "camp" ] = entity.getCamp()
		packDict[ "ename" ] = entity.getName()
		packDict[ "dbid" ] = entity.databaseID
		return packDict
	...
	def onLeaveCommon( self, selfEntity, baseMailbox, params ):
		...
		baseMailbox.client.camp_onLeaveFengHuoLianTianSpace()
	...
	def closeCampFengHuoRoom( self, selfEntity ):
		"""
		提前结束掉某场战争 由CampMgr 关闭所有房间
		"""
	...
	def callXianFengMonster( self, selfEntity, camp ):
		"""
		召唤先锋怪
		"""
		if selfEntity.queryTemp( "isCallingMonster", False ):
			return
		if not selfEntity.campCallMonsterDict.has_key( camp ):
			...
			for i in xrange(3):
				...
				if selfEntity.campCallMonsterDict[ camp ].has_key( roadString ) and selfEntity.campCallMonsterDict[ camp ][ roadString ] == 0:
		...
		if position:
			...
			g_objFactory.getObject( XIAN_FENG_GUAI_DICT[ flag ][ tempString ] ).createEntity( selfEntity.spaceID, pos, (0, 0, 0), { "spawnPos":tuple( pos ), "ownCamp":camp, "road":tempString } )
			selfEntity.decCampCallBoxAmount( camp, CAN_CALL_AMOUNT, tempString )
			selfEntity.noticePlayers( cschannel_msgs.CAMP_FENG_HUO_XIAN_FENG_IS_APPEARING% ( campName, csconst.g_road_info[ tempString ] ), [] )
	...
	def onRoleDie( self, role, killer ):
		...
		if killer.isEntityType( csdefine.ENTITY_TYPE_CAMP_XIAN_FENG ):
			role.getCurrentSpaceBase().cell.onRoleBeKill( killer.ownCamp, 0, role.getCamp(), role.databaseID )
		elif killer.isEntityType( csdefine.ENTITY_TYPE_ROLE ):
			role.getCurrentSpaceBase().cell.onRoleBeKill( killer.getCamp(), killer.databaseID, role.getCamp(), role.databaseID )
	...
	def showMatchInfo( self, selfEntity ):
		...
		for mailbox in selfEntity._players:
			if BigWorld.entities.has_key( mailbox.id ):
				BigWorld.entities[ mailbox.id ].camp_onFengHuoLianTianOver()
			else:
				mailbox.cell.camp_onFengHuoLianTianOver()
	...
	def kickAllPlayer( self, selfEntity ):
		...
		for mailbox in selfEntity._players:
			if BigWorld.entities.has_key( mailbox.id ):
				BigWorld.entities[ mailbox.id ].camp_leaveFengHuoLianTian()
			else:
				mailbox.cell.camp_leaveFengHuoLianTian()
