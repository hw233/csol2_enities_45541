...
class Skill_611701( Skill_Normal ):
	"""
	宠物被动技能:物理反震,将所受到的物理伤害,按一定的比例反射给对方
	"""
	def __init__( self ):
		"""
		"""
	...
	def init( self, dict ):
		...
		self.reboundProb = self._param4 # 默认为非对应性格
	...
	def attach( self, ownerEntity ):
		...
		if ownerEntity.isEntityType(csdefine.ENTITY_TYPE_PET) :
			...
			if petOwner.etype == "REAL" :
				...
				petOwner.entity.attachSkillOnReal(self)
	...
	def __addEffect(self,entity):
		#加上反震概率
		oldReboundProb = entity.queryTemp("rebound_damage_probability",0.0)
		entity.setTemp( "rebound_damage_probability", self.reboundProb + oldReboundProb)
			
		#加上反震比率
		oldReboundPercent = entity.queryTemp( "rebound_damage_percent", 0.0 )
		entity.setTemp( "rebound_damage_percent", self.reboundPercent + oldReboundPercent )
	...
	def __removeEffect(self,entity):
		...
		if newReboundProb >0.0 :
			entity.setTemp("rebound_damage_probability",newReboundProb)
		else :
			entity.popTemp("rebound_damage_probability")
			
		#去掉反震比率
		oldReboundPercent = entity.queryTemp( "rebound_damage_percent", 0.0 )
		newReboundPercent = oldReboundPercent - self.reboundPercent
		if newReboundPercent > 0.0:
			entity.setTemp( "rebound_damage_percent", newReboundPercent )
		else :
			entity.popTemp("rebound_damage_percent")