# -*- coding: gb18030 -*-
#
# $Id: Buff_65003.py,v 1.4 2008-02-28 08:25:56 kebiao Exp $
...
class Buff_65003( Buff_Normal ):
	"""
	example:buff存在时效内，单位任何一次成功的物理攻击，均为攻击者恢复相当于本次伤害的x%的生命值
	"""
	def __init__( self ):
		"""
		构造函数。
	...
	def springOnDamage( self, caster, receiver, skill, damage ):
		...
		if skill.getType() != csdefine.BASE_SKILL_TYPE_PHYSICS and skill.getType() != csdefine.BASE_SKILL_TYPE_PHYSICS_NORMAL:
			return
		hpAdd = int( damage * self._p1 )
		# 这里判断是因为要加入一条提示信息。
		if caster.HP_Max - caster.HP < hpAdd:
			hpAdd = caster.HP_Max - caster.HP
		if hpAdd > 0:
			# "%s恢复了你%i点生命值"
			caster.statusMessage( csstatus.SKILL_HP_BUFF_CURE, self.getName(), hpAdd )
			caster.addHP( hpAdd )
