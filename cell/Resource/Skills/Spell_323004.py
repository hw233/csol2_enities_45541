...
class Spell_323004( ActiveHomingSpell ):
	"""
	战士连击2
	"""
	def __init__( self ):
	...
	def onTick( self, caster ):
		...
		if len( self._targetActions ):
			...
			if targetAction == "hit":
				...
				direction.normalise()
				targetPos = self._target.getObject().position + direction * 1
				self._target.getObject().lineToPoint( targetPos, 9, False )
			elif targetAction == "hit_a":
				direction = Math.Vector3( self._target.getObject().position - caster.position )
				direction.normalise()
				targetPos = self._target.getObject().position + direction * 3
				self._target.getObject().lineToPoint( targetPos, 10.5, False )
			elif targetAction == "hit_c":
				if VehicleHelper.getCurrVehicleID( caster ):
					direction = Math.Vector3( self._target.getObject().position - caster.position )
					direction.normalise()
					targetPos = self._target.getObject().position + direction * 3
					self._target.getObject().lineToPoint( targetPos, 15.5, False )
				else:
					direction = Math.Vector3( caster.position - self._target.getObject().position )
					direction.normalise()
					targetPos = self._target.getObject().position + direction * 3
					self._target.getObject().lineToPoint( targetPos, 15.5, False )
