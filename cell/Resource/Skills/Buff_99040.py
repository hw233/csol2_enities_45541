...
class Buff_99040( Buff_Normal ):
	"""
	增加生命上限、物理攻击力、法术攻击力（英雄王座副本使用）
	公式 X=A*(Lv+C)+B
	"""
	def __init__( self ):
		"""
	...
	def doBegin( self, receiver, buffData ):
		...
		receiver.calcHPMax()
		# 物理攻击力
		damage_value = int( math.ceil( self._pA2 * ( self._level + self._pC2 ) + self._pB2 ) )
		receiver.damage_min_value += damage_value
		receiver.damage_max_value += damage_value
		receiver.calcDamageMin()
		receiver.calcDamageMax()
		# 法术攻击力
		magic_damage_value = int( math.ceil( self._pA3 * ( self._level + self._pC3 ) + self._pB3 ) )
