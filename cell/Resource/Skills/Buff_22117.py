...
class Buff_22117( Buff_Normal ):
	...
	def receive( self, caster, receiver ):
		...
		if len( buffs ) > 0:
			...
			receiver.statusMessage( csstatus.TAKE_EXP_SUCCESS, receiver.queryTemp( "rewardExpHour", 0 ), sexp )
	...
	def doAppend( self, receiver, buffIndex ):
		...
		if sk.getPercent() == self.getPercent():
			...
			if sk.isCharge != self.isCharge:
				if self.isCharge:
					...
					receiver.statusMessage( csstatus.TAKE_EXP_BUFF_EXIST, sexp1 )
				return
		...
		if isappend == 0:
			...
			receiver.statusMessage( csstatus.TAKE_EXP_SUCCESS2, math.ceil( isappend / 60.0 ), sexp )
		self.setRoleExpRecord( receiver )
