...
class Skill_611710( Skill_Normal ):
	"""
	宠物被动技能辉煌
	
	主人和自身使用法术攻击伤害对方时，
	有a%的几率，所造成伤害的b%转化为自身魔法的提高。
	勇敢型宠物吸魔几率提高c%，效果提高d%。
	"""
	def __init__( self ):
		"""
		...
		self.param1 = 0			# 触发吸魔的一般几率
		self.param2 = 0			# 触发吸魔的加强几率
		self.param3 = 0			# 一般吸魔比例
		self.param4 = 0			# 加强吸魔比例
		self.param5 = 0			# 能获得加强效果的性格
		self.effectPercent1 = 0	# 根据宠物性格确定的触发吸魔几率
		self.effectPercent2 = 0 # 根据宠物性格确定的吸魔比例
	...
	def springOnDamage( self, caster, receiver, skill, damage  ):
		...
		@type    damage: int32
		"""
		if skill.getType() != csdefine.BASE_SKILL_TYPE_MAGIC:
			return
		...
		if absorbValue > 0:
			caster.addMP( absorbValue )
			caster.statusMessage( csstatus.SKILL_PET_INBORN_SKILL_SPRING, self.getName() )
			caster.statusMessage( csstatus.SKILL_TARGET_MP_CURE, self.getName(), caster.getName(), absorbValue )
