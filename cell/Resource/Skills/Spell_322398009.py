...
from Love3 import g_honorItemJinbao
import ItemTypeEnum
import csstatus


class Spell_322398009( Spell_Item ):
	"""
	"""
	def __init__( self ):
		"""
	...
	def receive( self, caster, receiver ):
		...
		if item is None:
			ERROR_MSG( "荣誉度替换：player( %s )'s item is None.uid:%s." % uid )
			return
	...
	def useableCheck( self, caster, target ):
		...
		uid = caster.queryTemp( "item_using" )
		item = caster.getByUid( uid )
		dropType, dropInstance = g_honorItemJinbao.getDropData( item.getLevel() )
