...
class Spell_IntervalMissile( Spell ):
	"""
	间隔导弹技能
	"""
	def __init__( self ):
		...
		self.intervalTime = 0.0			# 导弹发射时间间隔
	...
	def cast( self, caster, target ):
		...
		for i in range( self.mount ):
			...
			i = float( i )
			# 导弹发射角度的计算
			# 发射角度 = 角色当前角度 + 导弹发射角度/2 - 导弹发射角度*每个导弹的偏移角度百分比
			if self.angle == CIR_ANGLE:		# 导弹发射角度为360度的特殊情况
				y = yaw + (math.pi*2) * (i/mount)
			else:
				try:
					y = yaw + (math.pi*2) * (self.angle/(CIR_ANGLE*2.0)) - (math.pi*2) * (self.angle/CIR_ANGLE) * (i/(mount-1.0))
				except:		# 导弹数量等于1的情况
					y = yaw
			...
			lifeTime += i * self.intervalTime	# 加上发射时间间隔
			trap.setLifeTime( lifeTime )
			trap.setDstPos( endDstPos )
			trap.delayLineToPoint( self.delayTime + i * self.intervalTime )	# 延迟发射
