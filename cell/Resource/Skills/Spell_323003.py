...
class Spell_323003( ActiveHomingSpell ):
	"""
	战士连击2
	"""
	def __init__( self ):
	...
	def onTick( self, caster ):
		...
		if len( self._targetActions ):
			...
			if targetAction == "hit_d_1" and not VehicleHelper.getCurrVehicleID( caster ):
				...
				direction = Math.Vector3( self._target.getObject().position - caster.position )
				direction.normalise()
				targetPos = self._target.getObject().position + direction * 6
				self._target.getObject().lineToPoint( targetPos, 30, False )
