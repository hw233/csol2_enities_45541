...
class Spell_323007( ActiveHomingSpell ):
	"""
	剑客连击3
	"""
	def __init__( self ):
	...
	def onTick( self, caster ):
		...
		if len( self._targetActions ):
			...
			if targetAction == "hit":
				...
				direction.normalise()
				targetPos = self._target.getObject().position + direction * 1.2
				self._target.getObject().lineToPoint( targetPos, 9, False )
			elif targetAction == "hit_a":
				direction = Math.Vector3( self._target.getObject().position - caster.position )
				direction.normalise()
				targetPos = self._target.getObject().position + direction * 4
				self._target.getObject().lineToPoint( targetPos, 35, False )
