...
import ECBExtend #add by wuxo 2012-5-7
import SkillTargetObjImpl
from bwdebug import *


class Spell_HomingChild( Spell_PhysSkillImprove ):
	"""
	物理连击子技能
	"""
	def __init__( self ):
	...
	def calcSkillHitStrength( self, source, receiver, dynPercent, dynValue ):
		...
		skilldamage = self.calcProperty( base, extra, dynPercent + source.skill_extra_percent / csconst.FLOAT_ZIP_PERCENT, dynValue + source.skill_extra_value )	
		return skilldamage
...
class Spell_TargetChangeHomingChild( Spell_HomingChild ):
	"""
	对自己施法的连击技能
	"""
	def __init__( self ):
	...
	def init( self, data ):
		...
		Spell_HomingChild.init( self, data )
	...
	def cast( self, caster, target ):
		...
		if self.casterMoveDistance and self.casterMoveSpeed:
			...
			if caster.__class__.__name__ != "Role":
				...
				caster.move_speed = self.casterMoveSpeed
				caster.updateTopSpeed()
				timeData = (dstPos - caster.position).length/self.casterMoveSpeed
				caster.addTimer( timeData, 0, ECBExtend.CHARGE_SPELL_CBID )
	...
	def receive( self, caster, receiver ):
		...
		@type  receiver: Entity
		"""
		if receiver.id != caster.id:
			Spell_HomingChild.receive( self, caster, receiver )
...
class Spell_HomingChild_Distance( Spell_HomingChild ):
	...
	def cast( self, caster, target ):
		...
		if self.casterMoveSpeed:
			targetObject = target.getObject()
			direction =  targetObject.position - caster.position
			if caster.__class__.__name__ != "Role": #怪物无视碰撞
				distance = direction.length - 0.5 # 防止重叠在一起
				direction.normalise()
				dstPos = caster.position + direction * distance  
				caster.moveToPosFC( dstPos, self.casterMoveSpeed, self.casterMoveFace )
			else:
				direction.normalise()
				distance = caster.distanceBB( targetObject )
