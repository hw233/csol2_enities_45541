# -*- coding: gb18030 -*-
#
# $Id: Buff_ShieldElement.py,v 1.0 9:15 2010-4-7 jiangyi Exp $
...
class Buff_1112( Buff_Shield ):
	"""
	example:火元素伤害吸收	吸收火伤害，最多吸收x点，护盾就会消失。	
	"""
	def __init__( self ):
		"""
		构造函数。
		...
		self._absortMax = 0 # 最多吸收x点
		self._elementDamageType = 0 # 元素伤害类型
		self._param = 0
	...
	def init( self, dict ):
		...
		self._absortMax = int( dict[ "Param1" ] if len( dict[ "Param1" ] ) > 0 else 0 )
		self._elementDamageType = int( dict[ "Param2" ] if len( dict[ "Param2" ] ) > 0 else 0 )
	...
	def doShield( self, receiver, damageType, damage ):
		...
		@rtype: 返回被消减后的伤害值
		"""
		if damageType != self._elementDamageType:
			return damage
	...
	def doBegin( self, receiver, buffData ):
		...
		buffData[ "skill" ] = self.createFromDict( { "param":{ "p1":self._absortMax } } )
