...
class MonsterXianFeng( Monster ):
	...
	def __init__( self ):
		...
		self.setEntityType( csdefine.ENTITY_TYPE_XIAN_FENG )
		self.integral = 0
	...
	def addIntegral( self, integral ):
		...
		self.getCurrentSpaceBase().cell.addTongFengHuoLianTianIntegral( self.ownTongDBID, integral )
	...
	def onDie( self, killerID ):
		...
		if killer.isEntityType( csdefine.ENTITY_TYPE_XIAN_FENG ) or killer.isEntityType( csdefine.ENTITY_TYPE_FENG_HUO_LIAN_TIAN_TOWER ) or killer.isEntityType( csdefine.ENTITY_TYPE_FENG_HUO_LIAN_TIAN_BASE_TOWER ):
			self.getCurrentSpaceBase().cell.addTongFengHuoLianTianIntegral( killer.ownTongDBID, self.integral )
		else:
			self.getCurrentSpaceBase().cell.addTongFengHuoLianTianIntegral( killer.tong_dbID, self.integral )
		self.getCurrentSpaceBase().cell.decTongFengHuoLianTianIntegral( self.ownTongDBID, self.integral )
