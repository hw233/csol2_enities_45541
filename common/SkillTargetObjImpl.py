...
class SkillTargetObjEntity( SkillTargetObjNone ):
	...
	def getObject( self ):
		...
		如果封装的是一个entity 那么返回entity ，封装的是position返回的是 类似(0,0,0)
		"""
		return BigWorld.entities.get( self._entityID, None )
	...
	def loadFromPacket( self, valDict ):
		...
		"""
		eid = valDict[ "param" ]
		if type( eid ) == int and BigWorld.entities.has_key( eid ):
			self._entityID = eid
		else:
			self._entityID = 0
...
class SkillTargetObjItem( SkillTargetObjNone ):
	...
	def getOwner( self ):
		...
		"""
		try:
			owner = BigWorld.entities[ self._ownerID ]
		except KeyError:
			return None
		return owner
	...
	def getObject( self ):
		...
		try:
			entity = owner.kitbags[1].getByTote( self._uid )
		except IndexError:
			ERROR_MSG( "Receiver %i uid %i not exist!" % owner.id, self._uid )
			return None
	...
	def addToPacket( self ):
		...
		打包自身需要传输的数据，数据必须是一个dict，具体参数详看SkillTargetObjImpl；
		"""
		return { "objType" : self.getType(), "param" :  ( self._uid, self._ownerID )  }
	...
	def loadFromPacket( self, valDict ):
		...
		@type  valDict: dict
		"""
		self._uid, self._ownerID = valDict[ "param" ]
...
class SkillTargetObjEntityPacket( SkillTargetObjNone ):
	...
	def addToPacket( self ):
		...
		打包自身需要传输的数据，数据必须是一个dict，具体参数详看SkillTargetObjImpl；
		"""
		return { "objType" : self.getType(), "param" :  self._entityIDs }

	def getObject( self ):
		"""
		"""
		entitys = []
		for eid in self._entityIDs:
	...
	def loadFromPacket( self, valDict ):
		...
		@type  valDict: dict
		"""
		self._entityIDs = list( valDict[ "param" ] )
...
#创建技能施展封装对象，物品
def createTargetObjItem( uid, ownerID ):
	if ownerID == 0:return None
	inst = SkillTargetObjItem()
	inst.init( uid, ownerID )
	return inst
...
_g_skillTargetObjClassMap[csdefine.SKILL_TARGET_OBJECT_ENTITYS]			= SkillTargetObjEntitys
_g_skillTargetObjClassMap[csdefine.SKILL_TARGET_OBJECT_POSITION]		= SkillTargetObjPosition
_g_skillTargetObjClassMap[csdefine.SKILL_TARGET_OBJECT_ITEM]			= SkillTargetObjItem
_g_skillTargetObjClassMap[csdefine.SKILL_TARGET_OBJECT_ENTITYPACKET]	= SkillTargetObjEntityPacket
