...
class TongCityWarMember:
	...
	def isSameType( self, obj ):
		return isinstance( obj, TongCityWarMember )
		
class TongCityWarInfo:
	# 帮会城市战副本数据
	def __init__( self ):
		self.tongDBID = 0
		self.integral = 0
	...
	def upInfoToClient( self, info ):
		for m in self.members.itervalues():
			# 把战报发给玩家
			if m.isIn:
				m.mailBox.client.tong_onCityWarReport( info.tongDBID, info.rname, info.kill, info.bekill, info.isIn )
	
	def upInfoAllToClient( self, playerMailbox ):
		for m in self.members.itervalues():
			playerMailbox.client.tong_onCityWarReport( m.tongDBID, m.rname, m.kill, m.bekill, m.isIn )
		
	def upIntegralToClient( self, tongDBID, integral ):
		# 更新帮会城战积分到客户端
		for m in self.members.itervalues():
			m.mailBox.client.tong_onUpdateCityWarPoint( tongDBID, integral )
	
	def upIntegralAllToClient( self, playerMailBox ):
		playerMailBox.client.tong_onUpdateCityWarPoint( self.tongDBID, self.integral )
	...
	def isSameType( self, obj ):
		return isinstance( obj, TongCityWarInfo )

class TongCityWarInfos:
	def __init__( self ):
		self.infos = {}
		self.winner = 0
	...
	def createObjFromDict( self, dict ):
		obj = TongCityWarInfos()
		obj.instance( dict )
		return obj
	
	def isSameType( self, obj ):
		return isinstance( obj, TongCityWarInfos )
		
cityWarMemberInstance = TongCityWarMember()
cityWarInfoInstance = TongCityWarInfo()
cityWarInfosInstance = TongCityWarInfos()