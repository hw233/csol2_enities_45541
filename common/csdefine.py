...
ROLE_INIT_DAOFA						= 14	# 初始化道法
ROLE_INIT_REWARD_QUESTS				= 15	# 初始化悬赏任务
...
ENTITY_TYPE_SPACE_SHUIJING_DOOR		= 39	# 水晶副本传送门
ENTITY_TYPE_XIAN_FENG				= 40	# 帮会夺城战复赛（烽火连天）先锋怪
ENTITY_TYPE_FENG_HUO_LIAN_TIAN_TOWER = 41	# 帮会夺城战复赛（烽火连天）防御塔
ENTITY_TYPE_FENG_HUO_LIAN_TIAN_ALTAR = 42	# 帮会夺城战复赛（烽火连天）祭坛
ENTITY_TYPE_FENG_HUO_LIAN_TIAN_BASE_TOWER = 43	# 帮会夺城战复赛（烽火连天）基地防御塔
ENTITY_TYPE_FENG_HUO_LIAN_TIAN_BATTLE_FLAG = 44	# 帮会夺城战复赛（烽火连天）战旗
ENTITY_TYPE_DANCE_KING					=45		# 舞王雕像(实现类似城主雕像)
ENTITY_TYPE_YI_JIE_ZHAN_CHANG_TOWER		= 46	# 异界战场箭塔
ENTITY_TYPE_DANCESEAT					= 47	# 舞厅坐位
ENTITY_TYPE_NPCDanceModel					= 48	# 舞厅中显示的角色模型
ENTITY_TYPE_CITY_WAR_FINAL_BASE			= 49	# 帮会夺城战决赛据点
ENTITY_TYPE_CAMP_XIAN_FENG				= 50	# 阵营烽火连天先锋怪
ENTITY_TYPE_CAMP_FENG_HUO_TOWER			= 51	# 阵营烽火连天防御塔
ENTITY_TYPE_CAMP_FENG_HUO_ALTAR			= 52	# 阵营烽火连天祭坛
ENTITY_TYPE_CAMP_FENG_HUO_BASE_TOWER	= 53	# 阵营烽火连天基地防御塔
ENTITY_TYPE_CAMP_FENG_HUO_BATTLE_FLAG	= 54	# 阵营烽火连天战旗
...
ENTITY_FLAG_CANT_BE_HIT_BY_ROLE			= 25			# 不可以被玩家攻击标志(双向)
...
ENTITY_FLAG_NPC_NAME					= 35			# 头顶名称强制显示为NPC的格式
ENTITY_FLAG_RAD_FOLLOW					= 36			# 游荡标志
ENTITY_VOLATILE_ALWAYS_CLOSE			= 37			# 坐标朝向同步一直关闭（用于防御塔等类型）
ENTITY_FLAG_CANT_BE_HIT_BY_MONSTER		= 38			# 不可以被怪物攻击标志（单向）
ENTITY_FLAG_CANT_BE_HIT_BY_ROLE_2		= 39			# 不可以被玩家攻击标志（单向）
ENTITY_FLAG_HEAD_ALWAYS_SHOW			= 40			# 头顶名称和血条一直显示
ENTITY_FLAG_RAD_FOLLOW_ACTION			= 41			# 匹配游荡动作标志
ENTITY_FLAG_ALAWAY_HIDE_NAME			= 42			# 屏蔽怪物名字
ENTITY_FLAG_ALAWAY_HIDE_LEVEL			= 43			# 屏蔽怪物等级
ENTITY_FLAG_ALAWAY_HIDE_HPBAR			= 44			# 屏蔽怪物血条
ENTITY_FLAG_CLOSE_FADE_MODEL			= 45			# 关闭怪物模型淡入淡出
ENTITY_FLAG_ALAWAY_LIE_DOWN				= 46			# monster临死状态，播放死亡动作
ENTITY_FLAG_ALAWAY_HIDE_CMAP			= 47			# 屏蔽怪物阵营显示
ENTITY_FLAG_MODEL_COLLIDE				= 48			# 客户端模型增加真实碰撞
ENTITY_FLAG_SPECIAL_BOSS				= 49			# 特殊boss，有这种标志位的在攻击玩家时会显示箭头在玩家头顶，以显示boss当前攻击目标。
ENTITY_FLAG_ALAWAY_HIDE_QUEST			= 50			# 屏蔽怪物任务标记
ENTITY_FLAG_SHOW_DAMAGE_VALUE			= 51			# 显示NPC伤害数值
ENTITY_FLAG_CANT_HIT_AND_BE_HIT_MONSTER	= 52			# 不可以被怪物攻击标志（双向）
...
ROLE_FLAG_FORBID_TOPSPEED		= 32	#禁止修改速度上限值标记
...
ENTITY_STATE_PICK_ANIMA					= 14			# 拾取灵气
...
#技能施法
SKILL_SPELL_INTONATE = 0       #吟唱施法
SKILL_SPELL_RECEIVE_DALAY_TIME = 1 #无吟唱条施法
...
BUFF_ORIGIN_YXLM_COEXISTENT				= 12			# 英雄联盟版的失落宝藏副本里NPC出售的装备的buff（相同ID的Buff可共存）
...
BUFF_INTERRUPT_LAND_VEHICLE				= 13			#打断陆行骑宠buff
BUFF_INTERRUPT_TELEPORT_FLY				= 14			#打断飞翔传送buff
...
RECEIVER_CONDITION_ROLE_WITH_COMPLETED_QUESTS = 0x1a	# 需要玩家完成指定任务
RECEIVER_CONDITION_ROLE_HAS_UNCOMPLETED_QUESTS = 0x1b	# 需要拥有其中一个未完成任务
...
SKILL_REQUIRE_TYPE_VITALITY				= 0x05				# 需要体力值
SKILL_REQUIRE_TYPE_HP   				= 0x06				# 需要生命值
...
SKILL_ID_SMART_PET_PHYSICS				= 2				# 敏捷型宠物普通物理攻击技能ID
SKILL_ID_INTELLECT_PET_MAGIC			= 3				# 智力型宠物普通魔法攻击技能ID
...
AI_TYPE_COMBO							=	6			# 连续AI类型 战斗状态下
AI_TYPE_COMBO_ACTIVERATE				= 	7			# 连续AI执行概率 战斗状态下
...
AI_EVENT_ENTER_GUARD_TRAP				=	40			# 有entity进入卫兵陷阱事件
...
KB_ZD_MAX_SPACE						= 20	# 证道系统单个包裹最大空间
KB_ZD_MAX_EQUIP_SPACE				= 8		# 证道系统角色道心最大空间
KB_ZHENG_DAO_ID						= 0x10	# 证道界面包裹
KB_COM_DAO_XIN_ID					= 0x11	# 道心界面普通包裹
KB_EQUIP_DAO_XIN_ID					= 0x12	# 角色道心界面包裹
...
QUEST_TYPE_CAMP_ACTIVITY			= 14	# 阵营活动任务
QUEST_TYPE_CAMP_DAILY				= 15	# 阵营日常任务

#reward quest bigType
REWARD_QUEST_TYPE_NONE				= 0		#未定义任务
REWARD_QUEST_TYPE_TONG				= 1		#帮会任务
REWARD_QUEST_TYPE_CAMP				= 2		#阵营任务
REWARD_QUEST_TYPE_DAILY				= 3		#每日任务

#reward quest smallType
REWARD_QUEST_TYPE_NONE						= 0		# 未定义
REWARD_QUEST_TYPE_TREASURE					= 1		# 宝藏任务
REWARD_QUEST_TYPE_POTENTIAL					= 2		# 潜能任务
REWARD_QUEST_TYPE_PLACARD					= 3		# 公告通缉任务
REWARD_QUEST_TYPE_CTG						= 4		# 闯天关任务
REWARD_QUEST_TYPE_DART						= 5		# 运镖任务
REWARD_QUEST_TYPE_ROB						= 6		# 劫镖任务
REWARD_QUEST_TYPE_MEMBER_DART				= 7 	# 成员运镖任务
REWARD_QUEST_TYPE_IMPERIAL_EXAMINATION 		= 8		# 科举考试
REWARD_QUEST_TYPE_MERCHANT					= 9		# 商会任务
REWARD_QUEST_TYPE_RUN_MERCHANT				= 10	# 跑商任务
REWARD_QUEST_TYPE_TONG_FETE					= 11	# 帮会祭祀
REWARD_QUEST_TYPE_TONG_BUILD				= 12	# 帮会建筑
REWARD_QUEST_TYPE_TONG_NORMAL				= 13	# 帮会日常
REWARD_QUEST_TYPE_CAMP_ACTIVITY				= 14	# 阵营活动任务
REWARD_QUEST_TYPE_CAMP_DAILY				= 15	# 阵营日常任务
REWARD_QUEST_TYPE_CRUSADE					= 16	# 每日讨伐任务
REWARD_QUEST_TYPE_MATERIAL					= 17	# 每日材料任务
REWARD_QUEST_TYPE_SPACE_COPY				= 18	# 每日副本任务
...
QUEST_OBJECTIVE_SUBMIT_EMPTY		= 43	# 提交物品（属性：空位）
QUEST_OBJECTIVE_NOT_SUBMIT_EMPTY	= 44    # 拥有物品，不用提交（属性：空位）
QUEST_OBJECTIVE_ADD_CAMP_MORALE		= 45	# 获得阵营积分
QUEST_OBJECTIVE_CAMP_KILL			= 46	# 阵营活动击杀怪物任务
QUEST_OBJECTIVE_VEHICLE_ACTIVED			= 47	# 激活一个骑宠
QUEST_OBJECTIVE_CAMPACT_DELIVER		= 48	# 阵营活动交付物品任务目标
QUEST_OBJECTIVE_CAMPACT_TALK		= 49	# 阵营活动对话任务
QUEST_OBJECTIVE_CAMPACT_EVENT_USE_ITEM	= 50	# 阵营活动使用物品任务
...
QUEST_REWARD_RATE_MONEY_FROM_ROLE_LEVEL	= 44	# 根据玩家等级按一定比率获的金钱奖励
QUEST_REWARD_RATE_EXP_FROM_ROLE_LEVEL	= 45	# 根据玩家等级按一定比率获的经验奖励
QUEST_REWARD_RATE_QUEST_PART_COMPLETED	= 46	# 完成一部分任务目标要给的奖励
QUEST_REWARD_MONEY_FROM_REWARD_QUEST_QUALITY = 47	# 根据悬赏任务品质获得不同的金钱奖励
QUEST_REWARD_EXP_FROM_REWARD_QUEST_QUALITY = 48	# 根据悬赏任务品质获得不同的经验奖励
QUEST_REWARD_TONG_NOMAL_MONEY		= 49	# 帮会金钱奖励
QUEST_REWARD_TONG_EXP				= 50	# 帮会经验奖励
QUEST_REWARD_ITEMS_FROM_CLASS		= 51	# 根据玩家职业给予不同的物品奖励
QUEST_REWARD_SKILL_FROM_CLASS		= 52	# 根据玩家职业给奖励不同的技能
QUEST_REWARD_SOLTS_MONEY			= 53	# 根据老虎机奖励倍数奖励金钱
QUEST_REWARD_SOLTS_EXP				= 54	# 根据老虎机奖励倍数奖励经验
QUEST_REWARD_SOLTS_POTENTIAL		= 55	# 根据老虎机奖励倍数奖励潜能值
QUEST_REWARD_SOLTS_DAOHENG			= 56	# 根据老虎机奖励倍数奖励道行值
...
QUEST_COMPLETE_RULE_DEFAULT			= 0		# 全部目标完成才算完成
QUEST_COMPLETE_RULE_PART_TASK_COM	= 1		# 只要一个目标完成就算任务完成
#####################################################################################
...
CHAT_CHANNEL_TONG_CITY_WAR			= 22			# 帮会战场频道
...
# about pk
# --------------------------------------------------------------------
PK_STATE_PROTECT					= 0x01	# 保护状态
PK_STATE_PEACE						= 0x02	# 白名状态
PK_STATE_ATTACK						= 0x04	# 褐名(攻击)状态(120秒内没有再次触发变为非攻击状态)
PK_STATE_REDNAME					= 0x08	# 红名状态
PK_STATE_BLUENAME					= 0x10	# 蓝名状态
PK_STATE_ORANGENAME					= 0x20	# 橙名状态（也就是小红名状态）
...
#pk模式
PK_CONTROL_PROTECT_PEACE				= 1		# 和平模式
PK_CONTROL_PROTECT_TEAMMATE				= 2		# 组队模式
PK_CONTROL_PROTECT_KIN					= 3		# 家族模式
PK_CONTROL_PROTECT_TONG					= 4		# 帮会模式
PK_CONTROL_PROTECT_RIGHTFUL				= 5		# 善意模式
PK_CONTROL_PROTECT_JUSTICE				= 6		# 正义模式
PK_CONTROL_PROTECT_NONE					= 7		# 全体模式
PK_CONTROL_PROTECT_TEMPORARY_FACTION	= 8		# 临时阵营模式 ( 目前只在异界战场中存在，处于同一临时阵营的玩家不可相互攻击 )
PK_CONTROL_PROTECT_LEAGUE				= 9		# 联盟模式（帮会夺城战决赛中以联盟为单位）
PK_CONTROL_PROTECT_CAMP					= 10	# 阵营模式

PK_CONTROL_PROTECT_ACT_1				= 20	#活动模式1
PK_CONTROL_PROTECT_ACT_2				= 21	#活动模式2
PK_CONTROL_PROTECT_ACT_3				= 22	#活动模式2
...
SPECIALSHOP_SUBTYPE_VEHICLE_PROPS	= 11		# 坐骑道具子类
...
CURRENCY_TYPE_MONEY		= 1			# 游戏币货币类型
CURRENCY_TYPE_SILVER		= 2			# 银元宝货币类型
CURRENCY_TYPE_GOLD		= 3			# 金元宝货币类型
...
REVIVE_BY_REVIVEPOINT				= 8		# 通过复活点数原地复活
...
SPACE_TYPE_YE_ZHAN_FENG_QI			= 38 	# 夜战凤栖镇
SPACE_TYPE_TONG_TURN_WAR			= 39	# 帮会车轮战
SPACE_TYPE_YXLM_PVP 				= 40	# 英雄联盟副本PVP
SPACE_TYPE_FENG_HUO_LIAN_TIAN 		= 41	# 帮会夺城战复赛（烽火连天）
SPACE_TYPE_TIAO_WU					= 42	# 跳舞地图
SPACE_TYPE_DESTINY_TRANS			= 43	# 天命轮回副本
SPACE_TYPE_MERCURY_CORE_MAP			= 44	# 水之核心副本
SPACE_TYPE_YI_JIE_ZHAN_CHANG		= 45	# 异界战场副本
SPACE_TYPE_AO_ZHAN_QUN_XIONG		= 46	# 鏖战群雄
SPACE_TYPE_DANCEHALL				= 47    # 舞厅
SPACE_TYPE_DANCECOPY_CHALLENGE		= 48	# 斗舞挑战副本
SPACE_TYPE_JUE_DI_FAN_JI			= 49	# 绝地反击活动副本
SPACE_TYPE_CAMP_TURN_WAR			= 50	# 阵营车轮战
SPACE_TYPE_DANCECOPY_PARCTICE		= 51	# 斗舞练习副本
SPACE_TYPE_CITY_WAR_FINAL			= 52	# 帮会夺城战决赛
SPACE_TYPE_CAMP_FENG_HUO_LIAN_TIAN	= 53	# 阵营烽火连天
SPACE_TYPE_TEMPLATE					= 54	# 副本模板
SPACE_TYPE_WM						= 55	# 位面地图
SPACE_TYPE_MELT_MONSTER_POT			= 56	# 炼妖壶副本
SPACE_TYPE_FANG_SHOU				= 57	# 防守副本
...
TONG_AMOUNT_MAX						= 1000	# 服务器能创建的帮会数量
TONG_AD_LENGTH_MAX 					= 40    # 帮会宣传字数最大

# 帮会职位
TONG_DUTY_MEMBER				= 1 # 帮众
TONG_DUTY_TONG					= 2 # 堂主
TONG_DUTY_DEPUTY_CHIEF			= 3 # 副帮主
TONG_DUTY_CHIEF					= 4 # 帮主

TONG_DUTY_CHIEF_SUBALTERN_COUNT	= 2		# 最大副帮主数量
TONG_DUTY_TONG_COUNT			= 4		# 最大堂主数量

TONG_DUTYS =[ TONG_DUTY_MEMBER, TONG_DUTY_TONG, TONG_DUTY_DEPUTY_CHIEF, TONG_DUTY_CHIEF ]
TONG_CITY_WAR_SPEAK = [TONG_DUTY_TONG, TONG_DUTY_DEPUTY_CHIEF, TONG_DUTY_CHIEF]		#帮会战争决赛频道发言职务

# 帮会权限
TONG_RIGHT_MEMBER_MANAGE		= 1 # 成员管理(招收、开除玩家)
TONG_RIGHT_AFFICHE				= 2 # 帮会公告
TONG_RIGHT_PET_SELECT			= 3 # 神兽选择
TONG_RIGHT_CHANGE_DUTY			= 4 # 职位变更
TONG_RIGHT_STORAGE_MANAGE		= 5 # 仓库权限设置
TONG_RIGHT_LEAGUE_MAMAGE		= 6 # 同盟管理
TONG_RIGHT_ACTIVITY				= 7 # 帮会活动( 魔物来袭、帮会祭祀、帮会掠夺战 )
TONG_RIGHT_SALARY_RATE			= 8 # 帮会俸禄兑换额
TONG_RIGHT_DISMISS_TONG			= 9 # 解散帮会
TONG_RIGHT_CALL_ALL_MEMBER		= 10 # 帮主令
TONG_RIGHT_GRADE_CHAT			= 11 # 听取帮会聊天
TONG_RIGHET_STORAGE_ITEM_ACCESS	= 12 # 帮会物品存取
TONG_RIGHT_MEMBER_SCHOLIUM		= 13 # 设置成员备注
TONG_RIGHT_ACTIVITY_ABA			= 14 # 帮会擂台
TONG_RIGHT_ACTIVITY_COMPETITION	= 15 # 帮会竞技
TONG_RIGHT_SET_AD				= 16 # 帮会广告
TONG_RIGHT_CHANGE_NAME			= 17 # 帮会改名
TONG_RIGHT_SIGN					= 18 # 帮会会标
TONG_RIGHT_ASSIGN_ITEM		= 19 #分配物品
...
CHANGE_MONEY_FISHING_JOY				= 112		# 玩家捕鱼
CHANGE_MONEY_JUE_DI_FAN_JI			= 113		# 绝地反击活动奖励金钱
CHANGE_MONEY_AO_ZHAN_QUN_XIONG		= 114		# 鏖战群雄活动奖励金钱
CHANGE_MONEY_CAMP_FENG_HUO			= 115		# 阵营烽火连天活动奖励金钱
...
CHANGE_POTENTIAL_TRANS				= 11		# 传功吸取潜能
CHANGE_POTENTIAL_PICK_ANIMA			= 12		# 拾取灵气玩活获取
...
TONG_CHANGE_MONEY_QTREWARD				= 	26	# 任务奖励
TONG_CHANGE_MONEY_REQUEST_FETE			=	27	# 开启帮会祭祀活动
TONG_CHANGE_MONEY_REQUEST_ROB_WAR		=	28	# 开启帮会掠夺战活动
TONG_CHANGE_MONEY_REQUEST_MONSTER_RAID	=	29	# 开启魔物来袭活动
TONG_CHANGE_MONEY_REQUEST_RACE			=	30	# 开启帮会赛马活动
TONG_CHANGE_MONEY_OPEN_DART_QUEST		=	31	# 开启帮会运镖任务
TONG_CHANGE_MONEY_OPEN_NORMAL_QUEST		= 	32	# 开启帮会日常任务
TONG_CHANGE_MONEY_BUY_SPECIAL_ITEM		= 	33	# 帮会购买特殊商品

#---------------------------------------------------------------------
#帮会经验改变原因
#---------------------------------------------------------------------
TONG_CHANGE_EXP_QTREWARD				= 1		# 任务奖励
TONG_CHANGE_EXP_MONSTER_RAID			= 2		# 魔物来袭
TONG_CHANGE_EXP_FETE					= 3		# 祭祀
TONG_CHANGE_EXP_RACE					= 4		# 帮会赛马
TONG_CHANGE_EXP_ROB_WAR					= 5		# 掠夺战
TONG_CHANGE_EXP_SIGN_IN					= 6		# 帮会签到
...
DELETE_ITEM_UP_STEP			       		=	112 # 骑宠升阶
DELETE_ITEM_VEHICLE_TO_ITEM				=	113 # 骑宠变回物品
DELETE_ITEM_TO_KITBAG					=	114	# 把道具转成背包
DELETE_ITEM_KITBAG_TO_ITEM				=	115	# 把背包转成道具
DELETE_ITEM_SWAP_KITBAG					=	116	# 交换两个背包的位置
DELETE_ITEM_GMCOMMAND					=	117	# GM命令删除
DELETE_ITEM_REWARD_QUEST				=	118	# 使用道具刷新悬赏任务
#-------------------------------------------------------------------------------
#获得物品的原因
#-------------------------------------------------------------------------------
ITEM_NORMAL								=	0	# 默认
...
ADD_ITEM_VEHICLE_TO_ITEM				=	127	# 骑宠变回物品
ADD_ITEM_TO_KITBAG						=	128	# 把道具转成背包
ADD_ITEM_KITBAG_TO_ITEM					=	128	# 把背包转成道具
ADD_ITEM_SWAP_KITBAG					=	129	# 交换两个背包的位置
ADD_ITEM_BY_TALK						=	130	# 对话奖励物品
...
#荣誉度改变的原因
#-------------------------------------------------------------------------------
HONOR_CHANGE_REASON_RECOVER				= 1		#荣誉值恢复
HONOR_CHANGE_REASON_ZHENG_DUO			= 2		#NPC争夺战
...
ACTIVITY_FLAGS_YE_ZHAN_FENG_QI			= 24	#夜战凤栖战场
ACTIVITY_FLAGS_YAYU_NEW					= 25	#新版拯救猰貐
ACTIVITY_FLAGS_DESTINY_TRANS			= 26	# 天命轮回副本
ACTIVITY_FLAGS_TOWER_DEFENSE			= 27	# 塔防副本
ACTIVITY_FLAGS_DUDUZHU					= 28	# 嘟嘟猪
...
CHANGE_EXP_DANCE							= 53	# 舞厅exp
CHANGE_EXP_DANCEKING						= 54	# 舞王exp
CHANGE_EXP_JUE_DI_FAN_JI					= 55	# 绝地反击活动经验奖励
CHANGE_EXP_AO_ZHAN_QUN_XIONG				= 56	# 鏖战群雄活动
CHANGE_EXP_CAMP_FENG_HUO					= 57	# 阵营烽火连天活动经验奖励
...
CHANGE_SILVER_FISHING_JOY			=	10	# 捕鱼达人
...
CHANGE_GOLD_ZD_ACTIVE_GRID			=	15	# 激活道心格子
CHANGE_GOLD_ZD_ACTIVE_GUIDE			=	16	# 元宝召唤导师
...
INVOICE_NEED_CAMP_HONOUR = 0x400	# 灵魂币换物品
...
#活动主类型
#-------------------------------------------------------------------------
ACTIVITY_PARENT_TYPE_QUEST				= 1				#任务类
ACTIVITY_PARENT_TYPE_SPACECOPY			= 2				#副本类
ACTIVITY_PARENT_TYPE_ITEM				= 3				#物品
ACTIVITY_PARENT_TYPE_MONEY				= 4				#金钱
ACTIVITY_PARENT_TYPE_SKILL				= 5				#技能
ACTIVITY_PARENT_TYPE_SKILL				= 5				#组织（包括帮会，队伍等）
...
ACTIVITY_YE_ZHAN_FENG_QI			=	98	#夜战凤栖战场
ACTIVITY_ZHENG_JIU_YA_YU_NEW		=	99	#新版拯救猰貐
ACTIVITY_TONG_FENG_HUO_LIAN_TIAN	=	100	#帮会夺城战复赛（烽火连天）
ACTIVITY_TOWER_DEFENSE				=	101 #塔防副本
ACTIVITY_TEAM_COMPETITION			=	102 #组队竞技
ACTIVITY_DRAGON						= 	103	#大头暴龙活动
ACTIVITY_BEFORE_NIRVANA				=	104 #10级剧情副本
ACTIVITY_PLOT_LV40					=	105 #40级剧情副本
ACTIVITY_PLOT_LV60					=	106 #60级剧情副本
ACTIVITY_TEAM_CHALLENGE				=	107 #组队擂台
ACTIVITY_TONG_TURN_WAR				=	108 #帮会车轮战
ACTIVITY_YING_XIONG_LIAN_MENG_PVP	=	109 #英雄联盟副本PVP
ACTIVITY_EXAMINATION_XIANGSHI		=	110 #科举：乡试
ACTIVITY_EXAMINATION_HUISHI			=	111 #科举：会试
ACTIVITY_EXAMINATION_DIANSHI		=	112 #科举：殿试
ACTIVITY_DENTITY_TRANS_COM			=	113	#天命轮回副本（普通模式）
ACTIVITY_TONG_CAMPAIGN				=	114	#帮会魔物来袭
ACTIVITY_YI_JIE_ZHAN_CHANG			=	115 #异界战场副本
ACTIVITY_AO_ZHAN_QUN_XIONG			=	116 #鏖战群雄
...
#活动参与方式
ACTIVITY_JOIN_ROLE		= 	1	#个人参与
ACTIVITY_JOIN_TEAM		= 	2	#队伍参与
ACTIVITY_JOIN_TONG		=	3	#帮会参与

#竞技类活动行为
ACTIVITY_COMPETITION_START			= 	0	#开启
ACTIVITY_COMPETITION_TEAM_POINT		=	1	#积分
ACTIVITY_COMPETITION_ADD_HONOR		=	2	#增加荣誉
ACTIVITY_COMPETITION_SUB_HONOR		=	3	#减少荣誉
ACTIVITY_COMPETITION_CHANGE_BOX		=	4	#兑换宝箱
ACTIVITY_COMPETITION_SIGN_UP		=	5	#报名/竞标
ACTIVITY_COMPETITION_ROLE_KILL		=	6	#杀死玩家
ACTIVITY_COMPETITION_TIME			=	7	#开启时间
ACTIVITY_COMPETITION_ADD_PERSONAL_SCORE	=	8	#增加个人竞技积分
ACTIVITY_COMPETITION_SUB_PERSONAL_SCORE	=	9	#减少个人竞技积分
...
#格斗点数
ROLE_CombatCount_MAX  = 5                             #最大格斗点数
ROLE_CombatCount_TIMMER                      = 1.0  #脱离战斗格斗点数自动衰减timmer
ROLE_CombatCount_TIMMER_VALUE                = 1  #脱离战斗格斗点数自动衰减值
...
TIME_RESET_MATCH_STATUS			= 3.0       # 有人点击离开副本后，重新加入副本组队有一个3秒的时延
...
#骑宠类型定义
VEHICLE_TYPE_LAND = 1 #陆行骑宠
VEHICLE_TYPE_FLY  = 2 #飞行骑宠
VEHICLE_UPDATE_REASON_LEVEL_UP = 1 #等级改变引起的属性改变
VEHICLE_UPDATE_REASON_STEP_UP  = 2 #升阶引起的属性改变
...
TONG_LEVEL_CHANGE_REASON_ADD_EXP		= 2		# 帮会经验增加
...
#-----------------------------------------------------------------------------
# 副本关闭原因
FENG_QI_CLOSE_REASON_MIN_PLAYER 	= 1 # 该地图人数太少
FENG_QI_CLOSE_REASON_MIN_LEVEL 		= 2 # 该等级人数太少
FENG_QI_CLOSE_REASON_TIME_OUT		= 3 # 到时候结束活动

# 天命轮回副本棋盘事件类型
CHESS_BOARD_EVE_NONE				= 0 # 无
CHESS_BOARD_EVE_MOVE				= 1 # 棋子移动
CHESS_BOARD_EVE_BUFF				= 2 # 添加Buff
CHESS_BOARD_EVE_BOSS				= 3 # 挑战Boss
CHESS_BOARD_EVE_DROP				= 4 # 多倍掉落

DESTINY_ENTER_GATE_SINGLE			= 1 # 单人进入
DESTINY_ENTER_GATE_TEAM				= 2 # 队伍进入

# 提示信息
DESTINY_TRANS_FAILED_GATE			= 1	# 通关失败
DESTINY_TRANS_FINISH_GATE			= 2	# 完成
DESTINY_TRANS_FIRST_NAME			= 3	# 第一名

#----------------------------------------------------------------------------
# 阵营活动类型
#----------------------------------------------------------------------------
CAMP_ACT_DESTROY_BASE		= 1		# 破坏据点
CAMP_ACT_OBTAIN_POINT		= 2		# 获得积分
CAMP_ACT_KILL_BOSS			= 3		# 击杀Boss
CAMP_ACT_OCCUPY_BASE		= 4		# 攻占据点
CAMP_ACT_INTERCEPT_HELPER	= 5		# 拦截支援部队
CAMP_ACT_LITTLE_WAR			= 6		# 小攻防
CAMP_ACT_BIG_WAR			= 7		# 全面战争

# -----------------------------------------------------------------------
# 客户端对话声音播放优先级
# -----------------------------------------------------------------------
GOSSIP_PLAY_VOICE_PRIORITY_DEFAULT	= 0				#默认语音
GOSSIP_PLAY_VOICE_PRIORITY_OPTION	= 1				#对话语音
GOSSIP_PLAY_VOICE_PRIORITY_QUEST		= 2				#任务语音

#悬赏任务刷新类型
REWARD_QUEST_SYSTEM_REFRESH			= 0				#系统刷新
REWARD_QUEST_LOW_ITEM_REFRESH		= 1				#低等道具刷新
REWARD_QUEST_HIGH_ITEM_REFRESH		= 2				#高等道具刷新

#悬赏任务品质类型
REWARD_QUEST_QUALITY_WHITE			= 1				#白色品质
REWARD_QUEST_QUALITY_BLUE			= 2				#蓝色品质
REWARD_QUEST_QUALITY_PURPLE			= 3				#紫色品质
REWARD_QUEST_QUALITY_GREEN			= 4				#绿色品质

#悬赏任务每天能够接取的总次数
REWARD_QUEST_CAN_ACCEPT_NUM			= 20			#悬赏任务每天能接受20次

#悬赏任务界面显示状态
REWARD_QUEST_CAN_ACCEPT				= 0				#可接取悬赏任务
REWARD_QUEST_ACCEPT					= 1				#已接取悬赏任务
REWARD_QUEST_COMPLETED				= 2				#已完成悬赏任务

#------------------------------------------------------------------------
# 仙魔论战奖励类型
#------------------------------------------------------------------------
TDB_JOIN_REWARD					= 1				# 参与奖
TDB_DAMAGE_REWARD				= 2				# 伤害奖
TDB_FIRST_DAMAGE_REWARD			= 3				# 首触奖
TDB_DEATH_REWARD				= 4				# 死亡奖励
TDB_CURE_REWARD					= 5				# 治疗奖励

#------------------------------------------------------------------------
# 鏖战群雄
#------------------------------------------------------------------------
#房间类型
AO_ZHAN_ROOM_TYPE_NO_FAILURE	= 1		#没有失败者
AO_ZHAN_ROOM_TYPE_HAS_FAILURE	= 2		#有失败者

#------------------------------------------------------------------------
# 异界战场
#------------------------------------------------------------------------
# 站场内三方阵营
YI_JIE_ZHAN_CHANG_FACTION_TIAN		= 1		#天方阵营
YI_JIE_ZHAN_CHANG_FACTION_DI		= 2		#地方阵营
YI_JIE_ZHAN_CHANG_FACTION_REN		= 3		#人方阵营

# 副本关闭原因
YI_JIE_CLOSE_REASON_TIME_END		= 1 	# 到结束活动时间
YI_JIE_CLOSE_REASON_ONE_FACTION_WIN = 2		# 一方阵营提前取得胜利

#------------------------------------------------------------------------
# 绝地反击活动
#------------------------------------------------------------------------
JUE_DI_FAN_JI_NOT_SIGN_UP				= 0				# 未报名阶段
JUE_DI_FAN_JI_HAS_SIGN_UP				= 1				# 已经报名等待阶段
JUE_DI_FAN_JI_HAS_MATCHED				= 2				# 已经匹配成功阶段
JUE_DI_FAN_JI_HAS_CONFIRM_ENTER			= 3				# 已经确定进入阶段
JUE_DI_FAN_JI_HAS_ENTERED				= 4				# 已经进入副本阶段
JUE_DI_FAN_JI_SHOW_RANK_LIST			= 5				# 结束查看榜单阶段
JUE_DI_FAN_JI_VICTORY_STATUS			= 2				# 绝地反击胜利状态
JUE_DI_FAN_JI_DRAW_STATUS				= 1				# 绝地反击平局状态
JUE_DI_FAN_JI_FAILED_STATUS				= 0				# 绝地反击失败状态
...
DANCER_NONE		= 0		#未上榜
DANCER_GOLDEN_RATITO 	= 10  	#金牌舞王经验倍率10
DANCER_SILVER_RATITO	= 5  	#银牌舞王经验倍率5
DANCER_COPPER_RATITO	= 3		#铜牌舞王经验倍率3
DANCER_CANDIDATE_RATITO	= 2		#候选舞王经验倍率2
DANCER_NONE_RATITO		= 1		#未上榜经验倍率
DANCETIMEOUTTIMER			= 4104
DANCEKINGTIMEOUTTIMER		= 4105
DANCEHALLPERSONLIMIT		= 20
DANCETIMELIMIT 				= 8*60*60
DANCECOPYTIMELIMIT			= 360  # 挑战斗舞副本持续时间
DANCEKINGRANGE				= 50  # 舞厅中可以看到npc的名字范围
...
CITY_WAR_FINAL_BASE_RESOURCE			= 1				# 资源据点
CITY_WAR_FINAL_BASE_BATTLE				= 2				# 战斗据点
CITY_WAR_FINAL_BASE_FLAG				= 3				# 插旗点
CITY_WAR_FINAL_BASE_HEROMONU			= 4				# 英灵碑
CITY_WAR_FINAL_BASE_FLAG_GUARD			= 5				# 护旗将
CITY_WAR_FINAL_LIGHT_WALL				= 6				# 光墙

# 据点归属
CITY_WAR_FINAL_FACTION_NONE				= 0				# 无归属
CITY_WAR_FINAL_FACTION_ATTACK			= 1				# 攻城方
CITY_WAR_FINAL_FACTION_DEFEND			= 2				# 守城方

# --------------------------------------------------------------------
# copyEvent  define( designed by wangzhen )
# --------------------------------------------------------------------
# 副本事件
COPY_EVENT_ON_BEGIN_STAGE				= 1				# 副本关卡开始
COPY_EVENT_ON_MONSTER_DIE				= 2				# 怪物死亡
COPY_EVENT_ON_ROLE_DIE					= 3				# 角色死亡
COPY_EVENT_ON_ENTER_COPY				= 4				# 玩家进入副本
COPY_EVENT_ON_LEAVE_COPY				= 5				# 玩家离开副本
COPY_EVENT_ON_TELEPORT_READY			= 6				# 地图加载完成，玩家可以移动
COPY_EVENT_ON_FIRST_ENTER_COPY			= 7				# 第一个玩家进入事件
COPY_EVENT_ON_TIMER						= 8				# 副本onTimer事件
# 炼妖壶副本事件
COPY_EVENT_MMP_ON_SPAWN_GOBLIN			= 100			# 刷小怪事件
COPY_EVENT_MMP_ON_EVIL_WIND				= 101			# 吹妖风事件
COPY_EVENT_MMP_ON_FLOOR_TRAP			= 102			# 地面陷阱事件
COPY_EVENT_MMP_ON_BEFORE_SPAWN_GOBLIN	= 103			# 即将刷怪事件
# 防守副本事件
COPY_EVENT_FANG_SHOU_ON_GEAR_STARTING	= 200			# 机关开启事件
COPY_EVENT_FANG_SHOU_ON_NPC_HP_CHANGED	= 201			# 防守NPC血量改变事件
COPY_EVENT_FANG_SHOU_ON_TOWER_CREATE	= 202			# 防守副本防御塔创建事件



#静态关系模式类型定义
RELATION_STATIC_CAMP							= 1				#普通阵营关系模式
RELATION_STATIC_CAMP_FENG_HUO					= 2				#阵营烽火连天副本模式
RELATION_STATIC_TONG_FENG_HUO_AND_TERRITORY		= 3				#帮会烽火连天副本以及帮会领地模式
RELATION_STATIC_TONG_CITY_WAR					= 4				#帮会城战副本模式
RELATION_STATIC_YXLM							= 5				#英雄联盟副本模式
RELATION_STATIC_YI_JIE_ZHAN_CHANG				= 6				#异界战场副本模式
RELATION_STATIC_TONG_CITY_WAR_FINAL				= 7				#帮会城战决赛副本模式



#动态（临时）关系模式类型定义
RELATION_DYNAMIC_PRESONAL_ANTAGONIZE_ID			= 1				#动态单个敌对ID关系
RELATION_DYNAMIC_PRESONAL_FRIEND_ID				= 2				#动态单个友好ID关系
RELATION_DYNAMIC_PRESONAL_ANTAGONIZE_DBID		= 3				#动态单个敌对DBID关系
RELATION_DYNAMIC_PRESONAL_FRIEND_DBID			= 4				#动态单个友好DBID关系
RELATION_DYNAMIC_TEAM_ANTAGONIZE				= 5				#动态敌对队伍关系
RELATION_DYNAMIC_TEAM_FRIEND					= 6				#动态友好队伍关系
RELATION_DYNAMIC_TONG_ANTAGONIZE				= 7				#动态敌对帮会关系
RELATION_DYNAMIC_TONG_FRIEND					= 8				#动态友好帮会关系
RELATION_DYNAMIC_COMBAT_CAMP_ANTAGONIZE			= 9				#动态敌对战斗阵营关系
RELATION_DYNAMIC_COMBAT_CAMP_FRIEND				= 10			#动态友好战斗阵营关系

#模型显示规则
VISIBLE_RULE_BY_PLANEID			= 1 #位面ID
VISIBLE_RULE_BY_FLAG			= 2	#标识位
VISIBLE_RULE_BY_FLASH			= 3 #闪屏
VISIBLE_RULE_BY_TEL_AND_TEST	= 4	#传送状态和测试状态
VISIBLE_RULE_BY_SHOW_SELF		= 5 #显示自己属性检查
VISIBLE_RULE_BY_SETTING_1		= 6 #Role用户配置
VISIBLE_RULE_BY_SETTING_2		= 7 #pet用户配置
VISIBLE_RULE_BY_WATCH			= 8 #观察者模型
VISIBLE_RULE_BY_PROWL_1			= 9 #PlayerRole,ownPet潜行模式
VISIBLE_RULE_BY_PROWL_2			= 10 #Role潜行模式
VISIBLE_RULE_BY_PROWL_3			= 11 #pet潜行模式
